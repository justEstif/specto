...
func (s *SyncService) SyncPlugin(ctx context.Context, userID uuid.UUID, pluginName string) (*SyncSummary, error) {
	...
	return s.executeSyncFlow(ctx, userID, pluginName, plugin, *creds, log)
}
...
// executeSyncFlow runs the shared sync pipeline (steps 4-11) after
// credentials have been resolved. Both SyncPlugin and SyncPluginWithFile
// delegate here after their credential-specific setup.
func (s *SyncService) executeSyncFlow(
	ctx context.Context,
	userID uuid.UUID,
	pluginName string,
	plugin SourcePlugin,
